...
def edit_record(request, id):
    ...
    for attrib in attribs:
        ...
        if len(attrib_value) == 0:
            ...
            attrib_value = getattr(attrib_value[0],
                dataTypeMap[attrib.datatype], None)
    ...
    if form.is_valid():
        for attribute in attribs:
            keyvalue = KeyValueStore.objects.filter(record=record,
                attribute=attribute)
            if attribute.datatype in ('String', 'Text', 'Email'):
                if len(keyvalue) == 0:
                    ...
                    keyvalue = keyvalue[0]
                    setattr(keyvalue, 'value_text',
                        form.cleaned_data[attribute.name])
